const Categories = {
    ...
    <div class="min-h-screen bg-gray-50 flex">
      <aside class="w-64 bg-white border-r border-gray-100 min-h-screen flex flex-col fixed top-0 left-0 h-full">
        ...
        <nav class="p-4 flex-1">
          ...
          <ul class="space-y-1">
            ...
            <li><router-link to="/categories" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-white shadow-sm" style="background: linear-gradient(135deg, #1e3a5f, #2d6a9f)">🗂 Kategori</router-link></li>
      ...
      <div class="flex-1 flex flex-col ml-64">
        <header class="bg-white border-b border-gray-100 px-8 py-4 flex justify-between items-center">
          <div>
            ...
            <p class="text-sm text-gray-400">Kelola kategori barang</p>
          </div>
          <button @click="openModal()" class="px-4 py-2 rounded-xl text-sm font-semibold text-white hover:opacity-90" style="background: linear-gradient(135deg, #1e3a5f, #2d6a9f)">+ Tambah Kategori</button>
        </header>
        <main class="flex-1 p-8">
          <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <table class="w-full text-sm">
              ...
              <tbody>
                <tr v-for="(cat, i) in categories" :key="cat.id" class="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                  ...
                  <td class="px-6 py-4">
                    <div class="flex gap-2">
                      <button @click="openModal(cat)" class="px-3 py-1.5 rounded-lg text-xs font-semibold bg-amber-50 text-amber-600 hover:bg-amber-100">Edit</button>
                      <button @click="deleteData(cat.id)" class="px-3 py-1.5 rounded-lg text-xs font-semibold bg-red-50 text-red-600 hover:bg-red-100">Hapus</button>
                    </div>
                  </td>
                </tr>
              </tbody>
      ...
      <div v-if="showModal" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-2xl p-8 w-full max-w-md shadow-2xl">
          <h3 class="text-lg font-bold text-gray-800 mb-6">{{ form.id ? 'Edit' : 'Tambah' }} Kategori</h3>
          <div class="mb-4">
            <label class="block text-sm font-semibold text-gray-600 mb-2">Nama Kategori</label>
          ...
          <div class="mb-6">
            ...
            <textarea v-model="form.description" rows="3" class="w-full border-2 border-gray-100 bg-gray-50 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-400 focus:bg-white transition-all resize-none"></textarea>
          </div>
  ...
  `,
  data() { return { categories: [], showModal: false, form: { id: null, name: '', description: '' }, BASE_URL: 'http://localhost/backend-api/public/api' } },
